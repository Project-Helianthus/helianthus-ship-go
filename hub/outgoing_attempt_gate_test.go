...
func TestAuthorizedAttemptsGateEveryPathAndEndpointFallback(t *testing.T) {
	...
	for index, expected := range want {
		...
		if permits[index].Decision != api.OutgoingAttemptDecisionPermit || calls[index].context == nil {
			t.Errorf("dial %d did not receive an authorized attempt context", index)
		}
...
func TestHubCancellationInterruptsContextBlockedGatedDialWithoutDeadlock(t *testing.T) {
	...
	for _, invalidation := range invalidations {
		t.Run(invalidation.name, func(t *testing.T) {
			...
			case <-invalidationDone:
			case <-time.After(2 * time.Second):
				t.Fatal("Hub cancellation API blocked while DialContext was running")
			}
